...
import { getLiveMarketplaceDeals } from "@/lib/deal-store";
...
export function HorizontalDealList({ title, subtitle, filterType, viewAllLink }: Props) {
  ...
  useEffect(() => {
    ...
    getLiveMarketplaceDeals({ limit: 50 }).then((allDeals) => {
      ...
      if (!cancelled) setDeals(allDeals.filter(filterFn).slice(0, 8));
    });
