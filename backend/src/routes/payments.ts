...
const createPaymentSchema = z.object({
  orderId: z.string(),
  payerPhone: z.string().regex(/^01[3-9]\d{8}$/, "Invalid Bangladesh phone number"),
});

const executePaymentSchema = z.object({
  paymentId: z.string(),
  otp: z.string().default("123456"),
  pin: z.string().default("12121"),
});
...
paymentsRouter.post("/bkash/create", requireAuth, async (req, res) => {
  try {
    ...
    const paymentId = `MOCK-BK-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`;
...
paymentsRouter.post("/bkash/execute", requireAuth, async (req, res) => {
  try {
    ...
    for (const item of order.items) {
      for (let i = 0; i < item.quantity; i++) {
        const voucherCode = `CPB-${crypto.randomBytes(3).toString("hex").toUpperCase()}-${Date.now().toString(36).toUpperCase()}`;
      ...
      await prisma.dealOption.update({
...
paymentsRouter.get("/bkash/status/:paymentId", requireAuth, async (req, res) => {
