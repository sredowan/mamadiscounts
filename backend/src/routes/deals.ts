...
dealsRouter.get("/", optionalAuth, async (req, res) => {
  try {
    const {
      q, category, city, area, status = "ACTIVE",
      sort = "popular", minPrice, maxPrice, minDiscount,
      lat, lng, radius = "5", // Default 5km radius
      page = "1", limit = "20",
    ...
    if (city) where.merchant = { city: { contains: city } };
    if (area) where.merchant = { ...where.merchant as object, area: { contains: area } };
    if (minPrice) where.dealPrice = { ...where.dealPrice as object, gte: parseFloat(minPrice) };
    if (maxPrice) where.dealPrice = { ...where.dealPrice as object, lte: parseFloat(maxPrice) };
    ...
    switch (sort) {
      case "price_asc": orderBy = { dealPrice: "asc" }; break;
      case "price_desc": orderBy = { dealPrice: "desc" }; break;
      case "discount": orderBy = { discountPercent: "desc" }; break;
      case "newest": orderBy = { createdAt: "desc" }; break;
      case "rating": orderBy = { ratingAvg: "desc" }; break;
      default: orderBy = { quantitySold: "desc" }; break; // popular
    }
    ...
    if (lat && lng) {
      ...
      const nearbyMerchants = await prisma.$queryRaw<{ id: string }[]>`
        SELECT id, (
          6371 * acos (
...
dealsRouter.get("/featured", async (_req, res) => {
  try {
...
dealsRouter.get("/:slug", optionalAuth, async (req, res) => {
  try {
    const slug = Array.isArray(req.params.slug) ? req.params.slug[0] : req.params.slug;
    if (!slug) return res.status(400).json({ error: "Deal slug is required" });
